-- ============================================================
-- 0011_fix_ambiguous_couple_id.sql
-- Corrige "column reference couple_id is ambiguous" (SQLSTATE 42702).
--
-- Causa: `create_couple_invite()` y `accept_couple_invite()` declaran
-- `couple_id` como columna de su RETURNS TABLE, lo que lo convierte en una
-- variable de salida implícita dentro del cuerpo de la función. Cualquier
-- referencia SIN calificar a la columna `couple_id` de una tabla (p.ej.
-- couple_members.couple_id) queda ambigua entre esa variable y la columna.
-- Fix: calificar con alias de tabla todas las referencias a couple_id.
-- ============================================================
...
begin
  ...
  set revoked_at = now()
  where ci.couple_id = v_couple_id and ci.accepted_at is null and ci.revoked_at is null;
